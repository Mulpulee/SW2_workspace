...
float dist_samples[_SAMPLE_COUNT];
int pivot = 0;
...
void loop() {
  ...
  } else if ((dist_raw == 0.0) || (dist_raw < _DIST_MIN)) {
  ...
  dist_ema = _EMA_ALPHA * dist_filtered + _EMA_ALPHA * dist_ema;
  
  dist_samples[pivot++] = dist_ema;
  if (pivot == _SAMPLE_COUNT) pivot = 0;

  dist_median = GetMedian(dist_samples);
...
float GetMedian(float *samples)
{
  float sorted[_SAMPLE_COUNT];
  for (int i = 0; i < _SAMPLE_COUNT; i++) sorted[i] = samples[i];

  for (int i = _SAMPLE_COUNT - 1; i >= 0; i--)
  {
    int maxindex = 0;
    for (int j = 0; j <= i; j++)
    {
  ...
  if (_SAMPLE_COUNT % 2 == 0) return (sorted[_SAMPLE_COUNT / 2] + sorted[(_SAMPLE_COUNT / 2) + 1]) / 2;
  else return sorted[(_SAMPLE_COUNT - 1) / 2];
}

...
#define _DIST_PEAK 200.0   // minimum distance to be measured (unit: mm)
...
void loop() { 
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      analogWrite(PIN_LED, 255);
  } else if (distance < _DIST_PEAK) {    // In desired Range
      analogWrite(PIN_LED, 255 * (_DIST_MIN - distance) / 100);  
  } else if (distance > _DIST_PEAK) {    // In desired Range
      analogWrite(PIN_LED, 255 * (distance - _DIST_MAX) / 100);  
  }
